...
# Modelled after real JAMB past questions & curriculum
# ──────────────────────────────────────────────────────────────────────────────
JAMB_QUESTION_BANK = {
    "Mathematics": [
        {"question":"If log₁₀2 = 0.3010, find log₁₀8.","options":["A) 0.__(0.9030)","B) 0.9030","C) 0.__(2.4030)","D) 2.4030"],"answer":"B","explanation":"8 = 2³, so log₁₀8 = 3 × log₁₀2 = 3 × 0.3010 = 0.9030.","topic":"Algebra and Equations","difficulty":"Medium"},
...
def init_model():
    ...
    if not api_key:
        return None  # Will fall back to question bank
    ...
    for mid in model_ids:
        try:
            ...
            model.generate_content("Say OK")  # quick connectivity test
            return model
        except Exception:
            continue

    return None  # all models failed – use offline bank
...
def get_bank_question(subject, difficulty, used_hashes):
    ...
    if not candidates:
        candidates = bank  # allow repeats as last resort
    if candidates:
...
def update_streak(last_date_str, current_streak):
    ...
    last = datetime.strptime(last_date_str, "%Y-%m-%d").date() if isinstance(last_date_str, str) else last_date_str
...
DEFAULTS = {
    "stage": "home",
    "mode": "quick",          # quick | cbt | topic
    "subject": "Mathematics",
    "difficulty": "Medium",
    "time_per_q": 120,
    "total_qs": 10,
    "questions": [],           # list of question dicts served so far
    "answers": [],             # list of user answers (str or None)
    "current_idx": 0,
    "show_feedback": False,
    "timer_start": None,
    "total_time": 0.0,
    ...
    "history": {},             # subject -> list of {topic, correct, diff, date}
    "unlocked_ids": [],
    "cbt_subjects": [],
    "cbt_per_subject": 10,
}
...
def show_home():
    ...
    st.markdown(f"### ⚙️ Configure: **{{'quick':'Quick Quiz','cbt':'CBT Simulation','topic':'Topic Focus'}[mode]}**")
...
def configure_cbt():
    ...
    if len(chosen) < 2:
        ...
        total_time = total * 1.5  # ~1.5 min per question like real JAMB
...
def configure_topic():
    ...
    if st.button("🚀 Begin Topic Drill", type="primary", use_container_width=True, key="start_topic"):
        st.session_state.subject = subj
        st.session_state.difficulty = diff
        st.session_state.time_per_q = None  # no timer
...
def show_quiz():
    ...
    if not st.session_state.show_feedback:
        ...
        if st.button("✅ Submit Answer", type="primary", use_container_width=True, key=f"submit_{idx}"):
            ...
            if is_correct:
                pass  # score calculated dynamically
...
def show_results():
    ...
    for i, (q, a) in enumerate(zip(qs, ans)):
        ...
        with st.expander(f"{icon} Q{i+1}: {q['question'][:80]}… ({subj_label} — {q.get('topic','')})"):
            st.markdown(f"**Your answer:** {a if a != '__EXPIRED__' else 'Time expired'}")
    ...
    for topic, data in sorted(topic_stats.items(), key=lambda x: x[1]["correct"]/max(x[1]["total"],1)):
        ...
        colour = "#059669" if m >= 70 else ("#d97706" if m >= 40 else "#dc2626")
...
def show_stats_summary():
    ...
    with c3:
        st.markdown('<div class="stat-card">', unsafe_allow_html=True)
        st.markdown(f"**🔥 Streak:** {st.session_state.streak} day{'s' if st.session_state.streak!=1 else ''}")
        st.markdown("</div>", unsafe_allow_html=True)
